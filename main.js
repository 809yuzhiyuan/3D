    ...
    <script>
        ...
        function createWorld() {
            ...
            if (houses.length > 0) {
                ...
                // 创建箱子DOM元素
                createChestElement(chestHouse.x, chestHouse.z - 30);
            }
        }

        function createChestElement(x, z) {
            const chestEl = document.createElement('div');
            chestEl.className = 'chest-icon';
            chestEl.textContent = '📦';
            chestEl.title = '箱子 - 右键打开';
            
            // 添加右键点击事件
            chestEl.addEventListener('contextmenu', (e) => {
                e.preventDefault(); // 阻止右键菜单
                if (currentState === STATE.PLAYING) {
                    // 检查玩家是否靠近箱子
                    const dist = Math.sqrt((player.pos.x - x)**2 + (player.pos.z - z)**2);
                    if (dist < 50) { // 在一定距离内才能打开箱子
        ...
        function updateChestVisibility() {
            chestElements.forEach(chest => {
                const dist = Math.sqrt((player.pos.x - chest.x)**2 + (player.pos.z - chest.z)**2);
                const visible = dist < 400; // 与房屋渲染距离一致
                chest.element.style.display = visible ? 'block' : 'none';
                
                if (visible) {
                    // 计算屏幕坐标
                    const worldPos = new THREE.Vector3(chest.x, 10, chest.z); // 箱子在y=10的位置
                    const screenPos = worldPos.clone().project(camera);
                    
                    const x = (screenPos.x * 0.5 + 0.5) * window.innerWidth;
                    const y = (-screenPos.y * 0.5 + 0.5) * window.innerHeight;
                    
                    chest.element.style.left = `${x - 15}px`; // 减去一半宽高以居中
                    chest.element.style.top = `${y - 15}px`;
                }
            });
        }
